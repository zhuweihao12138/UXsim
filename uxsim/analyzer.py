...
class Analyzer:
    ...
    def network(s, t=None, detailed=1, minwidth=0.5, maxwidth=12, left_handed=1, tmp_anim=0, figsize=(6,6), network_font_size=4, node_size=2):
        ...
        # 绘制节点
        node_size=0.5
        with open('matsim/facilities.csv', mode='r', encoding='utf-8') as file:
            # 创建CSV阅读器
            reader = csv.reader(file)
            next(reader)
            # 遍历CSV文件中的每一行
            for row in reader:
                plt.plot(row[1],row[2], "o", c="blue", ms=node_size, zorder=10)
    ...
    def network_pillow(s, t=None, detailed=1, minwidth=0.5, maxwidth=12, left_handed=1, tmp_anim=0, figsize=6, network_font_size=20, node_size=2, image_return=0):
        ...
        # 遍历CSV文件中的每一行
            for row in reader:
                x = float(row[1]) * coef - minx
                y = flip(float(row[2]) * coef - miny)  # 使用之前定义的 flip 函数来调整 y 坐标
                # 在 PIL 中，使用 ellipse 函数绘制圆形节点
                draw.ellipse((x - node_size, y - node_size, x + node_size, y + node_size),fill="blue")
